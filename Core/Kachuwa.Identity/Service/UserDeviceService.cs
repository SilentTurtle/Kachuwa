    ...
    public class UserDeviceService : IUserDeviceService
    {
        ...
        public async Task<DeviceVerificationStatus> VerifyDevice(long userId, long userDeviceId)
        {
            ...
            using (var db = (DbConnection)dbFactory.GetConnection())
            {
                ...
                await
                    db.ExecuteAsync("Update dbo.UserDevice Set IsVerified=1 where UserId=@UserId and UserDeviceId=@UserDeviceId", new
                    {
            ...
            using (var db = (DbConnection)dbFactory.GetConnection())
            {
                ...
                int exist = await
                    db.ExecuteScalarAsync<int>(
                        " if(@IsWeb=1) begin select 1 from dbo.UserDevice Where IsDeleted=0 and IsActive=1 and UserId=@UserId and DeviceId=@DeviceId and  IsWeb=1 " +
                        " end else begin select 1 from dbo.UserDevice Where IsDeleted=0 and IsActive=1 and UserId=@UserId and DeviceId=@DeviceId and IsMobile=1 end",
                        new
                        {
                            userDevice.Browser,
                ...
                return await db.QueryFirstOrDefaultAsync<UserDeviceStatus>(
                       ...
                       " Select @totaldevice=count(1) from dbo.UserDevice Where IsDeleted = 0 and IsVerified=1 and UserId=@UserId;" +
                       " Select @md=IsMobile from dbo.UserDevice Where IsDeleted = 0 and IsMobile=1 and IsVerified=1 and UserId=@UserId;" +
                       " if(@IsWeb=1) begin select @IsThisUnverifiedLogin=~IsVerified from dbo.UserDevice Where IsDeleted=0 and IsActive=1 and UserId=@UserId and DeviceId=@DeviceId and  IsWeb=1 " +
                       " end else begin select  @IsThisUnverifiedLogin=~IsVerified from dbo.UserDevice Where IsDeleted=0 and IsActive=1 and UserId=@UserId and DeviceId=@DeviceId and IsMobile=1 end" +
        ...
        public async Task<bool> RemoveDeviceAsync(long userDeviceId, long userId)
        {
            ...
            using (var db = (DbConnection)dbFactory.GetConnection())
            {
                ...
                await
                    db.ExecuteAsync("Update dbo.UserDevice Set IsDeleted=1,IsActive=0 where UserId=@UserId and UserDeviceId=@UserDeviceId", new
                        {
                            UserDeviceId = userDeviceId,
                            UserId = userId
                        }
