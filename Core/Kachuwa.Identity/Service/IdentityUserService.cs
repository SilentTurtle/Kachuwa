...
namespace Kachuwa.Identity.Service
{
    public class IdentityUserService : IIdentityUserService
    {
        ...
        public async Task<IdentityLogin> GetExternalUserAsync(string loginProvider, string providerKey)
        {
            ...
            {
                await db.OpenAsync();
                return await db.QuerySingleOrDefaultAsync<IdentityLogin>("select * from  dbo.IdentityUserLogin Where LoginProvider=@LoginProvider and ProviderDisplayName=@ProviderDisplayName and  ProviderKey=@ProviderKey",
        ...
        public async Task<bool> AddUserLoginProvider(IdentityLogin model)
        {
            ...
            {
                await db.OpenAsync();
                await db.ExecuteAsync("insert into dbo.IdentityUserLogin (LoginProvider,ProviderDisplayName,ProviderKey,UserId) values(@LoginProvider,@ProviderDisplayName,@ProviderKey,@UserId)", new { model.LoginProvider, model.ProviderDisplayName, model.ProviderKey, model.UserId });
        ...
        public async Task<bool> DeleteAllUserLoginProviders(long userId)
        {
            ...
            {
                await db.OpenAsync();
                await db.ExecuteAsync("delete from dbo.IdentityUserLogin Where UserId=@UserId", new { UserId = userId });
        ...
        public async Task<bool> DeleteUserLoginProvider(long userId, string provider)
        {
            ...
            {
                await db.OpenAsync();
                await db.ExecuteAsync("delete from dbo.IdentityUserLogin Where UserId=@UserId and LoginProvider=@LoginProvider ", new { LoginProvider = provider, UserId = userId });
