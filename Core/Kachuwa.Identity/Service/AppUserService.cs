...
namespace Kachuwa.Identity.Service
{
    public class AppUserService : IAppUserService
    {
        ...
        public async Task<bool> UpdateProfilePicture(long identityUserId, string imagePath)
        {
            ...
            {
                await db.OpenAsync();
                await db.ExecuteAsync("Update dbo.AppUser Set ProfilePicture=@Image where IdentityUserId=@identityUserId",
...
", new { showRecords });
        ...
        public async Task<IEnumerable<UserExportViewModel>> ExportAllUsersAsync(int organizationId)
        {
            ...
            {
                await db.OpenAsync();
                return await db.QueryAsync<UserExportViewModel>("select au.FirstName,au.LastName,au.Email,au.Username,au.PhoneNumber,o.CompanyName as Organization, au.GroupName from  dbo.AppUser au left join dbo.Organization as o on o.OrganizationId=au.OrganizationId" +
        ...
        public async Task<IEnumerable<UserExportViewModel>> ExportAllPackageUsersAsync(int organizationId, int packageId)
        {
            ...
            {
                await db.OpenAsync();
                return await db.QueryAsync<UserExportViewModel>("select au.FirstName,au.LastName,au.Email,au.Username,au.PhoneNumber, au.GroupName,oo.CompanyName as Organization  from  dbo.AppUser au inner join  dbo.UserPackage as o on o.OrganizationId=au.OrganizationId and o.UserId=au.IdentityUserId " +
                                                                " left join dbo.Organization as oo on oo.OrganizationId=au.OrganizationId Where  PackageId=@packageId and  (o.OrganizationId=@OrganizationId or @OrganizationId=0) and o.IsDeleted = 0 order by au.FirstName asc",
        ...
        public async Task<IEnumerable<UserPrintouViewModel>> GetAllPackageUser(int organizationId, int packageId)
        {
            ...
            {
                await db.OpenAsync();
                return await db.QueryAsync<UserPrintouViewModel>("select au.IdentityUserId, au.FirstName,au.LastName,au.Email,au.Username,au.PhoneNumber, au.GroupName,oo.CompanyName as Organization,[dbo].[udf_GetUserPackageNames](au.IdentityUserId) as PackageNames,oo.CompanyName as Organization  from  dbo.AppUser au inner join  dbo.UserPackage as o on o.OrganizationId=au.OrganizationId and o.UserId=au.IdentityUserId " +
                                                                " left join dbo.Organization as oo on oo.OrganizationId=au.OrganizationId " +
        ...
        public async Task<string> GetUserEmailById(long customerId)
        {
            ...
            {
                await db.OpenAsync();
                return await db.ExecuteScalarAsync<string>("select Email from dbo.AppUser where IdentityUserId=@IdentityUserId ",
        ...
        public async Task<bool> CheckEmailExists(string email)
        {
            ...
            {
                await db.OpenAsync();
               var status= await db.ExecuteScalarAsync<int>("Select 1 from dbo.IdentityUser where Email=@Email ",
                    new {  Email = email });
