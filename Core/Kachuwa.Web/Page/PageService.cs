    ...
    public class PageService : IPageService
    {
            ...
            using (var db = (DbConnection)dbFactory.GetConnection())
            {
                ...
                if (pageId == 0)
                {
                    return await db.QueryAsync<PagePermissionViewModel>(
                        @"Select [Name] as [RoleName], [Id] as [RoleId], 0 as [PagePermissionId], 0 as [PageId], 0 as [AllowAccess] From [dbo].[IdentityRole]",
                        new { PageId = pageId });
                }
                else
                {
                    return await db.QueryAsync<PagePermissionViewModel>(
                                           @"Select [ir].[Name] AS [RoleName], [ir].[Id] AS [RoleId], pp.[PagePermissionId], pp.[PageId], pp.[AllowAccessForAll], pp.[AllowAccess]
                                            From [dbo].[IdentityRole] ir Left Join [dbo].[PagePermission] pp on [ir].[Id] = [pp].[RoleId] AND pp.[PageId] = @PageId",
                                           new { PageId = pageId });
                }
            }
        }
        ...
        public async Task<bool> AddUpdatePagePermission(PageRolePermissionViewModel models)
        {
            try
            {
                ...
                {
                    await db.OpenAsync();
                    await db.ExecuteAsync("Delete From [dbo].[PagePermission] Where PageId = @PageId",
                        new { PageId = models.PageId });
                    models.AutoFill();
                    foreach (var pagePermissionRoleId in models.PagePermissionsRole)
                    {
                        await db.ExecuteAsync("Insert into [dbo].[PagePermission](PageId, AllowAccessForAll, AllowAccess, RoleId, AddedBy) Values(@PageId, @AllowAccessForAll, @AllowAccess, @RoleId, @AddedBy);", new
                        {
