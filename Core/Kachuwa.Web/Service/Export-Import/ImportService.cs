...
namespace Kachuwa.Web.Service
{
    public class ImportService : IImportService
    {
        ...
        public ImportService(IHostingEnvironment hostingEnvironment)
        {
                ...
                while (rows.MoveNext())
                {
                    ...
                    for (int j = 0; j < row.Cells.Count; j++)
                    {
                        ...
                        if (cell != null)
                        {
                            ...
                            if (PropDict.ContainsKey(headerRow.GetCell(j).ToString().ToUpper()))
                            {
                                //getting header value
                                var Info = PropDict[headerRow.GetCell(j).ToString().ToUpper()];
