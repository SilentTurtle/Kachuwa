...
namespace Kachuwa.Web
{
    public class PageService : IPageService
    {
        ...
        public async Task<bool> CheckPageExist(string url)
        {
            ...
            {
                await db.OpenAsync();
                var result = await db.QueryAsync<int>("Select 1 from dbo.Page Where IsDeleted=0 and URL=@URL", new { URL = url });
            ...
            using (var db = (SqlConnection)dbFactory.GetConnection())
            {
                ...
                var result =
                    await db.QueryFirstAsync<PageViewModel>(
                        "select p.PageId,p.Name,p.Url,p.UseMasterLayout,p.IsActive,p.IsPublished,s.SEOId,s.MetaDescription,s.MetaTitle,s.Image from dbo.Page as p left join dbo.Seo as s on p.PageId=s.PageId and s.SeoType='page' where  p.IsDeleted = 0 and p.PageId = @PageId",
                        new { PageId = pageId });
                return result;
            }
        }
        ...
        public async Task<bool> DeletePageAsync(int pageId)
        {
            ...
            {
                await db.OpenAsync();
                var result = await db.ExecuteAsync("Update dbo.Page Set IsDeleted=1, IsActive=0  Where PageId=@id", new { id = pageId });
                var seoresult = await db.ExecuteAsync("Update dbo.Seo Set IsDeleted=1, IsActive=0 Where  Url=@Url", new { Url = page.Url });
                return true;
            }
        ...
        public async Task<bool> MakeLandingPage(int pageId)
        {
            ...
            {
                await db.OpenAsync();
                var result = await db.ExecuteAsync("Update dbo.Page set IsDeleted=0, IsActive=0,Ispublished=0 Where Url='landing';  Update dbo.Page set Url='landing',IsDeleted=0, IsActive=1,Ispublished=1  Where PageId=@id", new { id = pageId });
                return true;
            }
        }
    }
