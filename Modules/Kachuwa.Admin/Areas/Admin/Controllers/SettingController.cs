   ...
   // [Authorize(PolicyConstants.PagePermission)]
    public class SettingController : BaseController
    {
        ...
        public async Task<IActionResult> Web()
        {
            ...
            ViewData["FormDataSource"] = await GetFormDataSources(_setting.TimeZoneOffset);
        ...
        public async Task<IActionResult> Web(Setting model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                TimeSpan ts = TimeSpan.Parse(model.TimeZoneOffset);
                if (ts.Hours > 0)
                ...
                try
                {
                    model.TimeZoneName = TimeZoneInfo.GetSystemTimeZones().SingleOrDefault(x => TimeSpan.Compare(x.BaseUtcOffset, ts) == 0)?.StandardName;
                ...
                ViewData["FormDataSource"] = await GetFormDataSources(model.TimeZoneOffset);
                return View(model);
            }
            else
            {
        ...
        private async Task<FormDatasource> GetFormDataSources(string baseUtcOffset)
        {
            var formDataSource = new FormDatasource();
            var timeZones = TimeZoneInfo.GetSystemTimeZones();
            baseUtcOffset = baseUtcOffset.Replace("+", "");
            formDataSource.SetSource("FormDatasource", timeZones.Select(x => new FormInputItem()
            {
                IsSelected = x.BaseUtcOffset == TimeSpan.Parse(baseUtcOffset),
                //Id = x.BaseUtcOffset,
