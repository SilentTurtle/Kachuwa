...
{
    public class FileBaseLogger : ILog,ILoggerService
    {
        ...
        public bool LogError(LogType logtype, Func<string> messageFunc, Exception exception = null)
        {
            try
            {
                _init();
                var log = new Log
                {
                    ...
                    Status = messageFunc(),
                    Error = exception==null?"": JsonConvert.SerializeObject(exception)
                };
                Logs.Add(log);
