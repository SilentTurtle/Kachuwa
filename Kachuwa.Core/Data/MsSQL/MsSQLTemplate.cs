namespace Kachuwa.Data
{
    ...
    public class MsSQLTemplate : ISQLTemplate
    {
        ...
        public string PaginatedSql
            =>
                "SELECT COUNT(1) OVER() AS RowTotal, {SelectColumns}FROM {TableName} {join} {WhereClause} Order By {OrderBy} OFFSET {PageNumber}-1 ROWS FETCH NEXT {RowsPerPage} ROWS ONLY"
            ;
