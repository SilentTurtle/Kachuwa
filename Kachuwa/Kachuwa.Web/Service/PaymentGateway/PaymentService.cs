...
namespace Kachuwa.Web.Service
{
    public class PaymentService : IPaymentService
    {
        ...
        public PaymentService(IKeyGenerator keyGenerator, IFileOptions fileOptions, IStorageProvider storageProvider, ILogger logger, IWebHostEnvironment hostingEnvironment)
        {
        ...
        public async Task<int> InsertOrSave(PaymentGateway model)
        {
            ...
            using (var db = (SqlConnection)dbFactory.GetConnection())
            {
                ...
                await db.OpenAsync();
                return await db.ExecuteScalarAsync<int>("if exists( select 1 from paymentgateway where SystemName=@SystemName)" +
                    " BEGIN update paymentgateway set IsActive=@IsActive where SystemName=@SystemName; select paymentgatewayid from paymentgateway where SystemName=@SystemName END  " +
                    " ELSE BEGIN  insert into paymentgateway(PaymentGatewayName,Image,Description,SystemName,[Type],IsActive,AddedOn,AddedBy) " +
